...
export default function WorkflowPage() {
  ...
  const onNodeMouseDown = (e: React.MouseEvent, chId: string, nodeId: string) => {
    ...
    setDragging({ nodeId, chId, ox: e.clientX - (rect?.left ?? 0) - node.x, oy: e.clientY - (rect?.top ?? 0) - node.y });
  };
                  ...
                  <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
                    ...
                    <div className="flex-shrink-0 flex items-center gap-2 px-4 py-2 border-b" style={{ borderColor: 'var(--border-color)', background: 'var(--bg-surface)' }}>
                      <div className="relative">
                        ...
                        {showAddNode && (
                          <div className="absolute left-0 top-full mt-1 z-30 w-60 rounded-2xl overflow-hidden"
                      ...
                      <button
                        ...
                        onClick={() => { if (selectedNodeId) { removeNode(ch.id, selectedNodeId); setSelectedNodeId(null); } }}
                        ...
                        style={{ background: 'var(--bg-elevated)', border: `1px solid ${selectedNodeId ? 'rgba(220,38,38,0.4)' : 'var(--border-color)'}`, color: selectedNodeId ? '#DC2626' : 'var(--text-muted)' }}
                    ...
                    >
                      ...
                      {cv.nodes.map(node => {
                        ...
                        return (
                          <div key={node.id}
                            style={{ position: 'absolute', left: node.x, top: node.y, width: NODE_W, height: NODE_H, cursor: 'grab', userSelect: 'none', zIndex: dragging?.nodeId === node.id ? 10 : selectedNodeId === node.id ? 5 : 1 }}
                          ...
                          >
                            ...
                            <div className="w-full h-full rounded-2xl flex flex-col justify-center px-3 gap-1"
                              style={{ background: cs.bg, border: selectedNodeId === node.id ? cs.border.replace('2px', '3px') : cs.border, boxShadow: dragging?.nodeId === node.id ? 'var(--shadow-lg)' : selectedNodeId === node.id ? `0 0 0 3px ${cs.border.replace('2px solid ', '')}55, 0 4px 16px rgba(0,0,0,0.15)` : 'none' }}>
                              <div className="flex items-center gap-2">
                                <span className="text-base leading-none">{node.icon}</span>
                                <p className="font-display text-xs truncate flex-1" style={{ color: 'var(--text-primary)', fontWeight: selectedNodeId === node.id ? 800 : 600 }}>{node.label}</p>
                                <button
