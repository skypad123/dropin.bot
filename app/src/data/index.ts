import type { Instance, ConnectedApp, Workspace, AgentTeam, KnowledgeBase, Channel, ChannelType, SkillTag, ToolConfigField } from '../types';
...
export const INITIAL_INSTANCES: Instance[] = [
 ...
 { id: '5', name: 'Writing Buddy', model: 'Mistral',  status: 'running', uptime: '98.8%', conversations: '890',  integrations: ['Web'],                    usage: 34,  lastActive: '15m ago', temperature: 1.2, personality: 'Creative writing companion.',                         systemPrompt: 'You are a creative writing assistant.',              region: 'asia-pacific', environment: 'production',  workspaceId: 'ws-1', memories: { ...DEFAULT_MEMORIES },                        plugins: { ...DEFAULT_PLUGINS, imageGeneration: true }, skills: ['Copywriting', 'Editing', 'Brainstorming'] },
 { id: '6', name: 'DevOps Bot',    model: 'Claude 3', status: 'error',   uptime: '62.3%', conversations: '4.2K', integrations: ['Slack'],                  usage: 100, lastActive: '5m ago',  temperature: 0.4, personality: 'Infrastructure monitoring assistant.',               systemPrompt: 'You are a DevOps monitoring assistant.',             region: 'us-west',      environment: 'production',  workspaceId: 'ws-1', memories: { organiser: 'zep' },   plugins: { ...DEFAULT_PLUGINS, webSearch: true }, skills: ['Monitoring', 'Alerting', 'Incident Response'] },
];
...
export const CHANNEL_META: Record<ChannelType, { label: string; icon: string; color: string; bg: string; fields: { key: string; label: string; placeholder: string; secret?: boolean }[] }> = {
  slack:        { label: 'Slack',      icon: '💬', color: '#4A154B', bg: 'rgba(74,21,75,0.10)',    fields: [{ key: 'botToken', label: 'Bot Token', placeholder: 'xoxb-...', secret: true }, { key: 'signingSecret', label: 'Signing Secret', placeholder: 'abc123...', secret: true }] },
  discord:      { label: 'Discord',    icon: '🎮', color: '#5865F2', bg: 'rgba(88,101,242,0.10)',  fields: [{ key: 'botToken', label: 'Bot Token', placeholder: 'MTI3...', secret: true }, { key: 'guildId', label: 'Server ID', placeholder: '123456789' }] },
  'web-widget': { label: 'Web Widget', icon: '🌐', color: 'var(--burnt-orange)', bg: 'rgba(192,86,64,0.10)', fields: [{ key: 'allowedOrigins', label: 'Allowed Origins', placeholder: 'https://yoursite.com' }, { key: 'primaryColor', label: 'Brand Color', placeholder: '#C05640' }] },
  whatsapp:     { label: 'WhatsApp',   icon: '📱', color: '#25D366', bg: 'rgba(37,211,102,0.10)',  fields: [{ key: 'phoneId', label: 'Phone Number ID', placeholder: '123456...' }, { key: 'accessToken', label: 'Access Token', placeholder: 'EAAb...', secret: true }] },
  api:          { label: 'REST API',   icon: '⚡', color: '#7C3AED', bg: 'rgba(124,58,237,0.10)',  fields: [{ key: 'webhookUrl', label: 'Webhook URL', placeholder: 'https://...' }, { key: 'apiKey', label: 'API Key', placeholder: 'sk-...', secret: true }] },
  email:        { label: 'Email',      icon: '✉️', color: '#0EA5E9', bg: 'rgba(14,165,233,0.10)',  fields: [{ key: 'inboundEmail', label: 'Inbound Address', placeholder: 'agent@dropin.bot' }, { key: 'smtpHost', label: 'SMTP Host', placeholder: 'smtp.example.com' }] },
  telegram:     { label: 'Telegram',   icon: '✈️', color: '#26A5E4', bg: 'rgba(38,165,228,0.10)',  fields: [{ key: 'botToken', label: 'Bot Token', placeholder: '123456:ABC-...', secret: true }] },
  teams:        { label: 'MS Teams',   icon: '🟦', color: '#6264A7', bg: 'rgba(98,100,167,0.10)',  fields: [{ key: 'appId', label: 'App ID', placeholder: 'xxxxxxxx-...' }, { key: 'appPassword', label: 'App Password', placeholder: '...', secret: true }] },
};
